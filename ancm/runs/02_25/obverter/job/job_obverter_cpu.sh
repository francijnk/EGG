...
OUTPUT_DIR=$HOME/runs/02_25/obverter/
CUBLAS_WORKSPACE_CONFIG=:4096:2
# python3 -m ancm.data.obverter -d 4 --n_samples_train 160 --n_img 100 --resolution 64 
...
for i in {0..10}; do
    for ((j=32*i+1; j<=32*(i+1); j++)); do
        if ((j <= 80)); then
            ...
            python -m ancm.train --wandb_group 02_25_obverter  \
