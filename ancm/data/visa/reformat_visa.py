...
def reshape_make_tensor(data_concepts, n_distractors, n_features, n_samples, data_distractors=None):
    ...
    n_samples_adjusted = n_samples if data_concepts.shape[0] > 200 else n_samples // 10

    data_concepts = np.array(data_concepts.iloc[:,1:].values, dtype='int')
    data_reshaped = torch.empty((len(data_concepts)*n_samples_adjusted, n_distractors + 1, n_features), dtype=torch.float32)
    for concept_i in range(len(data_concepts)):
        category = categories.iloc[concept_i]  # data_concepts.iloc[concept_i,0]
        for sample_j in range(n_samples_adjusted):
...
def reformat(n_distractors, n_samples):
    np.random.seed(42)
    visa = pd.read_csv("ancm/data/visa/visa-homonyms.csv")
    ...
    # divide 80% for train, 10% test and valid
    train_features, temp_features, train_textlabels, temp_labels = train_test_split(features, textlabels, test_size=0.2)
