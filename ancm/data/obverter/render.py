...
def render_scene(filename, object_type, color, location, rotation, resolution):
    ...
    scene.render(filename, width=resolution, height=resolution, antialiasing=0.9999)


def get_object_fname(color, object_type, idx=None, n_img=None):
    assert not (idx is None and n_img is None)
    if idx is None:
        idx = np.random.randint(0, n_img)
    assets_dir = os.path.join(current_dir, 'assets')
    all_images = os.listdir(assets_dir)
    prefix = f'{color}_{object_type}_{idx}_'
    if not any(fname.startswith(prefix) for fname in all_images):
        return None
    else:
        return next(fname for fname in all_images if fname.startswith(prefix))
